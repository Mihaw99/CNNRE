...
class SemEvalDateset():
    ...
    def __load_data(self):
        ...
        print("data_loaddata: ", len(data), "label_loaddata: ", len(label))
        return data, labels
...
class SemEvalDataLoader(object):
    ...
    def __collate_fn(self, batch):
        # 四个输入参数，分别如下：
        # SemEvalDataLoader; list[128](data[0]为list[1]); list[128](label[0]为numpy.ndarray); mindspore._c_dataengine.CBatchInfo
        ...
        # data = Tensor.from_numpy(np.concatenate(data, axis=0))
        # label = Tensor.from_numpy(np.asarray(label, dtype=np.int64))
        return data, label
    ...
    def __get_data(self, filename, shuffle=False):
        ...
        loader = ds.GeneratorDataset(
            source=dataset,
            # column_names=['id','relation','head','tail','subj_start','subj_end','obj_start','obj_end','sentence','comment'],
        ...
        loader = loader.batch(
            batch_size=self.config.batch_size,
            # per_batch_map=self.__collate_fn
        )
        # loader.batch(128)

#         print("batch_size: ", loader.get_batch_size())
#         for step, (data, label) in enumerate(loader.create_tuple_iterator()):
#             if step == 0:
#                 print("data_shape1: ", data.shape, "label_shape1: ", label.shape)
#             else:
#                 break

        return loader, loader0
...
if __name__ == '__main__':
    ...
    for step, (data, label) in enumerate(test_loader.create_tuple_iterator()):
        ...
        minn = ops.ArgMinWithValue()
        maxx = ops.ArgMaxWithValue()
        min_v_pos1 = minn(pos1).item()
        max_v_pos1 = maxx(pos1).item()
        min_v_pos2 = minn(pos2).item()
        max_v_pos2 = maxx(pos2).item()
        min_v = min(min_v, min_v_pos1)
        min_v = min(min_v, min_v_pos2)
        max_v = max(max_v, max_v_pos1)
        max_v = max(max_v, max_v_pos2)
    print(min_v, max_v)
